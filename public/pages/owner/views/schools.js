...
export default async function schools({ refresh }) {
  ...
  return list.map((x) => {
    ...
    const price = input({ type: "number", min: 0, step: "0.01", value: x.subscription_price ?? 0 });
    const grace = input({ type: "number", min: 0, max: 120, value: x.grace_days ?? 14 });
    ...
    return panel(x.name, badge(...STATUS[x.status]),
      x.auto_suspended_at ? sub("أُوقفت تلقائيًا لانتهاء الاشتراك") : null,
      ...
      h("div", { class: "row" }, field("السعر السنوي", price), field("مدة السماح (أيام)", grace)),
      h("div", { class: "row", style: "justify-content:flex-start" },
        btn("حفظ الاشتراك", patch({ plan: plan.value, max_students: Number(max.value), subscription_end: end.value || null,
          subscription_price: Number(price.value), grace_days: Number(grace.value) }, "تم حفظ الاشتراك"), "sm"),
        x.status === "active"
