...
r.post("/student", limits.studentKey, handle(async (req, res) => {
  res.json(await inSchool(req, "ولي أمر", async (q, tenant) => {
    ...
    const attendance = await q("SELECT day, status FROM attendance WHERE student_id = $1 ORDER BY day DESC LIMIT 90", [s.id]);
    ...
    return {
      ...
      attendance, grades, teachers, announcements: news, fees,
    };
  }));
}));
